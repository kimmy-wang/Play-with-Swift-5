...
class Ball : UIView {
    ...
    lazy var mass: CGFloat = {
        return 2 * .pi * (self.frame.size.width/2.0) * (self.frame.size.width/2.0)
    }()
    ...
    class func randomBallInRect( rect: CGRect ) -> Ball{
        ...
        return Ball(frame: CGRect.init(x: centerx-radius, y: centery-radius, width: 2*radius, height: 2*radius), color: randomColor , elasticity: randomElasticity )
