...
struct Location{
    ...
    init?(coordinateString: String){
        ...
        if let commaIndex = coordinateString.range(of: ",")?.lowerBound{
            ...
            if let firstElement = Double(coordinateString[..<commaIndex]){
                
                if let secondElement = Double(coordinateString[coordinateString.index(after: commaIndex)..<coordinateString.endIndex]){
